...
use crate::models::{ExecutionRequest, ExecutionResult, NetworkAccess, ResourceUsage};
...
impl SandboxRunner {
    ...
    async fn execute_in_sandbox(&self, request: &ExecutionRequest) -> McpResult<ExecutionResult> {
        ...
        let seccomp_profile = match &request.sandbox_config.network_access {
            NetworkAccess::None => self
                .seccomp_manager
                .get_profile_path(SeccompProfileType::Basic),
            _ => self
                .seccomp_manager
                .get_profile_path(SeccompProfileType::Network),
        };
        ...
        let mut cmd = bubblewrap.build_command(&sandbox_config, &request.command, &request.args);
        ...
        let output = match timeout(timeout_duration, cmd.output()).await {
            ...
            Err(_) => {
                error!(
                    "bubblewrap command execution timed out: {} seconds",
                    request.timeout
                );
                return Err(McpError::Execution(format!(
