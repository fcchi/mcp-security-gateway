...
mod tests {
    ...
    fn test_error_counters() {
        ...
        if !stats.contains_key("auth") || !stats.contains_key(&error_code_key) {
            println!(
                "Counter presence check failed: auth={}, code={}",
                stats.contains_key("auth"),
                stats.contains_key(&error_code_key)
            );
        ...
        if auth_count == 0 || code_count == 0 {
            println!(
                "Count value check failed: auth={}, code={}",
                auth_count, code_count
            );
        ...
        if auth_count != code_count {
            println!(
                "Count value mismatch: auth={}, code={}",
                auth_count, code_count
            );
        ...
        if direct_count != auth_count {
            println!(
                "Direct count mismatch: direct={}, auth={}",
                direct_count, auth_count
            );
    ...
    fn test_handle_with_details() {
        ...
        let result: Result<i32, McpError> =
            Err(McpError::InvalidRequest("Input error".to_string()));
