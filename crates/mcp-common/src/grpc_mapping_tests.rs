...
#[cfg(test)]
mod tests {
    use crate::error::{error_code, ErrorDetail, ErrorResponse, McpError};
    use crate::grpc::{get_error_code_from_status, get_status_code_from_error_code, IntoStatus};
    use serde_json::json;
    ...
    fn test_all_error_codes_have_status_mapping() {
        ...
        for error_code in get_all_error_codes() {
            ...
            assert_eq!(
                status_code,
                matrix.get_status(error_code),
                "エラーコード {} のステータスマッピングが期待値と異なります",
                error_code
            );
        }
    }
    ...
    fn test_all_status_codes_map_to_sensible_error_code() {
        ...
        for status_code in status_codes {
            ...
            if !expected_errors.is_empty() {
                assert!(
                    expected_errors.contains(&error_code)
                        || get_status_code_from_error_code(error_code) == status_code,
                    "ステータスコード {:?} のエラーマッピングが期待値と異なります",
                    status_code
                );
            }
        }
    ...
    fn test_mcp_error_to_status_mapping() {
        ...
        let errors = vec![
            (
                McpError::Auth("認証エラー".to_string()),
                Code::Unauthenticated,
            ),
            (
                McpError::InvalidRequest("無効なリクエスト".to_string()),
                Code::InvalidArgument,
            ),
            (
                McpError::NotFound("リソースなし".to_string()),
                Code::NotFound,
            ),
            (
                McpError::PolicyViolation("ポリシー違反".to_string()),
                Code::PermissionDenied,
            ),
            (
                McpError::Sandbox("サンドボックスエラー".to_string()),
                Code::FailedPrecondition,
            ),
            (
                McpError::Execution("実行エラー".to_string()),
                Code::Internal,
            ),
            (McpError::Internal("内部エラー".to_string()), Code::Internal),
            (
                McpError::Temporary("一時エラー".to_string()),
                Code::Unavailable,
            ),
            (
                McpError::ExternalService("外部サービスエラー".to_string()),
                Code::Unavailable,
            ),
        ];
